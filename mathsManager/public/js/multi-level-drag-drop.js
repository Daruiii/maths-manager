...
class MultiLevelDragDrop {
  ...
  enableReorderMode(
    ...
    const sortableConfig = {
      ...
      onEnd: (evt) =>
        this.onDragEnd(evt, reorderRoute, level, itemClass, crossContainer, moveRoute),
    };
  ...
  buildReorderPayload(orderData, level, container = null) {
    switch (level) {
      case 'chapter':
        return {
          class_id: container
            ? this.extractContainerId(container.id, 'chapter')
            : this.getCurrentClassId(),
          chapter_orders: orderData,
        };
      case 'subchapter':
        return {
          chapter_id: container
            ? this.extractContainerId(container.id, 'subchapter')
