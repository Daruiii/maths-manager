      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <section className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 flex flex-col">
          <header className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-comfortaa-bold">
          ...
          <div className="space-y-4">
            {correctionRequests?.data?.length > 0 ? (
               correctionRequests.data.map((req: any) => (
                 <div key={req.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl hover:bg-gray-100 transition group border border-transparent hover:border-admin-color/20">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-text-gray">Reçu le {new Date(req.created_at).toLocaleDateString()}</p>
                        </div>
                    </div>
                    <a 
              ...
              <p className="text-center py-8 text-text-gray font-comfortaa italic">Aucune demande en cours 🎉</p>
            )}
          </div>
        </section>
        ...
        <section className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 flex flex-col">
          ...
          <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
            {ds?.length > 0 ? (
              ds.map((homework: any) => (
                <div key={homework.id} className="flex items-center justify-between p-4 border-b border-gray-50 last:border-0 group">
                    <div>
               ...
               <p className="text-center py-8 text-text-gray font-comfortaa italic">Aucun devoir actif</p>
            )}
          </div>
        </section>
