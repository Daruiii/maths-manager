...
export function useProblemSearch(routeName: string, initialSort: ProblemSort) {
  const [filters, setFilters] = useState<Filters>(INITIAL_FILTERS);
  const [sort, setSort] = useState<ProblemSort>(initialSort);
  ...
  const fetchPage = useCallback(
    async (pageNum: number, append: boolean) => {
      ...
      try {
        const res = await axios.get<PaginatedResponse<PickableProblem>>(route(routeName), {
          params,
        });
  ...
  return {
    ...
    setClassId: (classId: string) => setFilters((f) => ({ ...f, classId })),
    setDifficulty: (difficulty: string) => setFilters((f) => ({ ...f, difficulty })),
    setHarder: (harder: boolean) => setFilters((f) => ({ ...f, harder })),
    setYear: (year: string) => setFilters((f) => ({ ...f, year })),
    setAcademy: (academy: string) => setFilters((f) => ({ ...f, academy })),
    setSort,
