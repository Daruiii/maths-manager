...
import { DMPickerTab } from '@/Constants/dm';
...
export interface DMActiveSearch {
  items: PickableItem[];
  total: number;
  searchValue: string;
...
export function useDMExercisePicker({ multipleChapters, subchapters, privateTags = [] }: Options) {
  const [tab, setTab] = useState<DMPickerTab>('problems');
  const [isFiltersOpen, setIsFiltersOpen] = useState(false);

  const problemSearch = useDMProblemSearch();
  const exerciseSearch = useDMExerciseSearch();
  const privateSearch = useDMPrivateSearch();
  ...
  const activeSearch = useMemo((): DMActiveSearch => {
    if (tab === 'problems') {
      return {
