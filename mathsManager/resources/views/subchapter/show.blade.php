    ...
    <div class="container mx-auto">
            ...
            <div class=" md:p-4 flex flex-col items-center justify-center w-full" id="exercises-container">
                @foreach ($exercises as $index => $ex)
                    <div x-data="{ showClue: false, showSolution: false }"
                        class="exercise mb-8 rounded-lg box-shadow shadow-xl w-full md:w-10/12 {{ $ex->is_hidden ? 'bg-gray-100' : 'bg-white' }}"
                                        ...
                                        <div class="flex items-center space-x-2">
                                            ...
                                            @if ($ex->solution)
                                                <a href="{{ route('exercise.whitelist.show', $ex->id) }}"
                                                    class="inline-flex items-center px-2 py-1 border border-blue-300 rounded text-xs leading-4 font-medium text-blue-700 bg-blue-50 hover:bg-blue-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors"
                                                    title="Gérer les corrections">
                                                    🔒
                                                </a>
                                            @endif
                                            <x-button-edit href="{{ route('exercise.edit', ['id' => $ex->id]) }}" />
                                            <x-button-delete href="{{ route('exercise.destroy', $ex->id) }}" entity="cet exercice"
                                                entityId="exercise{{ $ex->id }}" />
                                            <form action="{{ route('exercise.toggleHidden', $ex->id) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit"
                                                    class="inline-flex items-center px-2 py-1 border rounded text-xs leading-4 font-medium transition-colors {{ $ex->is_hidden ? 'border-green-300 text-green-700 bg-green-50 hover:bg-green-100' : 'border-yellow-300 text-yellow-700 bg-yellow-50 hover:bg-yellow-100' }}"
                                                    title="{{ $ex->is_hidden ? 'Afficher l\'exercice' : 'Masquer l\'exercice' }}">
                                                    {{ $ex->is_hidden ? '👁️' : '🙈' }}
                                                </button>
                                            </form>
                                        </div>
                                        ...
                                        <div class="flex items-center space-x-4">
                                            @if ($ex->clue)
                                                <button @click="showClue = !showClue" class="flex row text-xs font-bold">
                                                    Indice
                                                    <svg :class="{ 'rotate-180': !showClue }" class="transition-transform" width="15px"
                                            ...
                                            @if ($ex->solution)
                                                @if (Auth::user()->role === 'admin' || Auth::user()->role === 'teacher' || $ex->isWhitelisted(Auth::id()))
                                                    <button @click="showSolution = !showSolution" class="flex row text-xs font-bold">
                                                        Correction
                                                        <svg :class="{ 'rotate-180': !showSolution }" class="transition-transform" width="15px"
                                                            height="15px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                            <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round">
                                                            </g>
                                                    ...
                                                    <div class="text-xs text-red-600 font-bold flex items-center justify-end w-full space-x-1">
                                                        ...
                                                        @if (Auth::user()->role === 'student')
                                                            @if ($ex->hasWhitelistRequest(Auth::id()))
                                                                <span
                                                                    class="text-xs text-yellow-600 font-normal bg-yellow-100 px-2 py-1 rounded whitespace-nowrap">
                                                                    ⏳ En cours
                                                                </span>
        ...
        <script>
            document.getElementById('reorder-button').addEventListener('click', function () {
                var exercises = document.querySelectorAll('.exercise:not(.bg-gray-100)'); // Exclut les masqués
                var minOrder = parseInt(exercises[0].dataset.order);
                new Sortable(document.getElementById('exercises-container'), {
                    animation: 150,
                    filter: '.bg-gray-100', // Empêche de drag les exercices masqués
                    // Update the order in the database when an item is moved
                    onEnd: function (evt) {
                        // Get only visible exercises (non-hidden)
                        var exercises = document.querySelectorAll('.exercise:not(.bg-gray-100)');
                        ...
                        <form id="requestForm" method="POST">
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="message" id="message" rows="3" maxlength="500"
                                    ...
                                    placeholder="Ex: Je n'arrive pas à comprendre cette méthode..."></textarea>
                                <p class="text-xs text-gray-500 mt-1">Un message aide l'équipe à mieux comprendre votre besoin.
                                </p>
                            </div>
