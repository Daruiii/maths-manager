...
@section('content')
    ...
    <section class="form-wrapper slide-left">
        <div class="form">
            ...
            {{-- texte au centre en pas tres grand qui affiche : {{ $selectedChapter->title }} --}}
            <p class="text-center form-explain mb-5 bg-gray-200">Chapitre choisi : {{ $selectedChapter->title }}</p>

            {{-- afficher les erreurs --}}
            <p class="form-explain mb-5"> Ici, vous pouvez créer une fiche d'exercices pour un utilisateur spécifique. </p>
            ...
            <form action="{{ route('exercises_sheet.store') }}" method="POST" id="exercisesSheetForm">
                                ...
                                @foreach ($exercises as $exercise)
                                    <div class="multiselect__option m-0">
                                        ...
                                        <span class="multiselect__label comfortaa-light text-sm p-2 rounded">
                                            {{ $exercise->name }} #{{ $exercise->order }}</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                ...
                <button type="submit">Créer la fiche d'exercices</button>
            </form>
