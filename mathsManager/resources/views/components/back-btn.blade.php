@props(['path'])
{{-- theme or if he is null blue-500 --}}
@props(['theme'])

<a href="{{ $path }}" class="flex row items-center gap-1 text-xs md:absolute md:top-24 md:right-4 md:black md:font-bold md:p-2 md:rounded mt-4 p-2 black rounded hover:bg-gray-200 transition-colors duration-300 ease-in-out">
<svg width="16px" height="16px" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path fill="#000000" d="M224 480h640a32 32 0 1 1 0 64H224a32 32 0 0 1 0-64z"></path><path fill="#000000" d="m237.248 512 265.408 265.344a32 32 0 0 1-45.312 45.312l-288-288a32 32 0 0 1 0-45.312l288-288a32 32 0 1 1 45.312 45.312L237.248 512z"></path></g></svg>    
 Retour
</a>

